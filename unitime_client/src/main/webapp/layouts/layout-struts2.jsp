...
<head>
	...
	<title>UniTime <%=Constants.VERSION%>| <% String __langT = Localization.getLanguage(); if (__langT != null && __langT.startsWith("ar")) { %>تطبيق الجدولة الجامعية<% } else { %><tiles:getAsString name="title" /><% } %></title>
    ...
    <span class='main'><span class='body' id="unitime-Page">
    	<span class="unitime-PageHeader" id="unitime-Header">
    		<span class="row">
    			...
    			<span class="content">
					<%-- UJ: title localized inline because adding a GwtMessages key would require editing upstream JavaSource --%>
					<span id='UniTimeGWT:Title' class="title"><% String __lang = Localization.getLanguage(); if (__lang != null && __lang.startsWith("ar")) { %>تطبيق الجدولة الجامعية<% } else { %><tiles:getAsString name="title" /><% } %></span>
					<s:if test="#request.showMenu">
